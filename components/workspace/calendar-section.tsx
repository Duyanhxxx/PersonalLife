  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-gray-500">Lịch kế hoạch</p>
          <h3 className="mt-2 text-2xl font-semibold text-gray-900">{data.monthLabel}</h3>
        </div>
        <div className="flex gap-2">
          <Link className="rounded-2xl border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900" href={`/app/calendar?month=${addMonth(data.monthKey, -1)}`}>Trước</Link>
          <Link className="rounded-2xl border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900" href={`/app/calendar?month=${addMonth(data.monthKey, 1)}`}>Tiếp</Link>
        </div>
      </div>
      <MonthGrid
      ...
      <div className="grid gap-4 lg:grid-cols-[1.1fr_1fr]">
        <form action={createPlannerEvent} className="rounded-[2rem] border border-gray-200 bg-white p-6 shadow-sm">
          <p className="text-sm font-semibold text-gray-900">Thêm sự kiện</p>
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            ...
            <select className="h-11 rounded-2xl border border-gray-300 px-3 text-sm" name="tone">
              <option value="blue">Làm việc sâu</option>
              <option value="green">Thói quen</option>
              <option value="amber">Chiến lược</option>
              <option value="purple">Bán thời gian</option>
              <option value="red">Khẩn cấp</option>
            </select>
            <input className="h-11 rounded-2xl border border-gray-300 px-3 text-sm" name="notes" placeholder="Ghi chú ngắn" />
          </div>
          <button className="mt-4 rounded-2xl bg-gray-900 px-4 py-2 text-sm font-medium text-white" type="submit">Lưu sự kiện</button>
        </form>
        <div className="rounded-[2rem] border border-gray-200 bg-white p-6 shadow-sm">
          <p className="text-sm font-semibold text-gray-900">Danh sách sắp tới</p>
