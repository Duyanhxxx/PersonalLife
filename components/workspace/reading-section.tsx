...
import { createĐang đọcBook, deleteĐang đọcBook, updateĐang đọcStatus } from "@/actions/reading-books";
import type { Đang đọcBook } from "@/types/section-data";

type Đang đọcSectionProps = {
  books: Đang đọcBook[];
};

const lanes: Đang đọcBook["status"][] = ["to_read", "reading", "finished"];
const labels = {
  to_read: "Sẽ đọc",
  reading: "Đang đọc",
  finished: "Đã xong",
...
export function Đang đọcSection({ books }: Đang đọcSectionProps) {
  return (
    <section className="space-y-4">
      <form action={createĐang đọcBook} className="rounded-[2rem] border border-gray-200 bg-white p-6 shadow-sm" encType="multipart/form-data">
              ...
              {books.filter((book) => book.status === lane).map((book) => (
                <article className="rounded-[1.5rem] border border-gray-200 bg-gray-50 p-4 shadow-sm" key={book.id}>
                  ...
                  <div className="mt-3 flex gap-2">
                    <form action={updateĐang đọcStatus}>
                    ...
                    <form action={deleteĐang đọcBook}>
                      <input name="id" type="hidden" value={book.id} />
