    ...
    <section className="space-y-4">
      <form action={createReadingBook} className="rounded-[2rem] border border-gray-200 bg-white p-6 shadow-sm" encType="multipart/form-data">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          ...
          <input className="h-11 rounded-2xl border border-gray-300 px-3 text-sm" name="author" placeholder={dict.author} />
            ...
            <div className="mt-4 space-y-4">
              {books.filter((book) => book.status === lane).map((book) => (
                <article className="rounded-[1.5rem] border border-gray-200 bg-gray-50 p-4 shadow-sm" key={book.id}>
                  ...
                  <p className="mt-1 text-sm text-gray-700">{book.author || (locale === "vi" ? "Tác giả không rõ" : "Unknown author")}</p>
                  <div className="mt-3 flex gap-2">
                    <form action={updateReadingStatus}>
                      ...
                      <button className="ml-2 text-xs font-medium text-gray-900" type="submit">{dictionary.common.save}</button>
                    </form>
