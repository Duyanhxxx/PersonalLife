  ...
  return (
    <section className="space-y-4">
      <form action={createReadingBook} className="rounded-[2rem] border border-[#8EE4AF]/50 bg-white/85 p-6 shadow-sm" encType="multipart/form-data">
        <p className="text-sm font-semibold text-[#05386B]">Add book</p>
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          <input className="h-11 rounded-2xl border border-[#8EE4AF] px-3 text-sm" name="title" placeholder="Book title" required />
          <input className="h-11 rounded-2xl border border-[#8EE4AF] px-3 text-sm" name="author" placeholder="Author" />
          <select className="h-11 rounded-2xl border border-[#8EE4AF] px-3 text-sm" name="status">
            <option value="to_read">To read</option>
            <option value="reading">Reading</option>
            <option value="finished">Finished</option>
          </select>
          <input accept="image/*" className="h-11 rounded-2xl border border-[#8EE4AF] px-3 py-2 text-sm" name="cover" type="file" />
        </div>
        <button className="mt-4 rounded-2xl bg-[#05386B] px-4 py-2 text-sm font-medium text-[#EDF5E1]" type="submit">Upload book</button>
      </form>
                ...
                <article className="rounded-[1.5rem] border border-[#8EE4AF]/40 bg-[#F7FBF4] p-4 shadow-sm" key={book.id}>
                  <div className="aspect-[3/4] overflow-hidden rounded-2xl bg-[#DDEEE5]">
                    {book.cover_url ? (
                      ...
                      <div className="flex h-full items-center justify-center text-sm text-[#379683]">No cover</div>
                    )}
                  </div>
                  <p className="mt-3 font-semibold text-[#05386B]">{book.title}</p>
                  <p className="mt-1 text-sm text-[#20555F]">{book.author || "Unknown author"}</p>
                  <div className="mt-3 flex gap-2">
                    <form action={updateReadingStatus}>
                      <input name="id" type="hidden" value={book.id} />
                      <select className="rounded-xl border border-[#8EE4AF] bg-white px-2 py-1 text-xs" defaultValue={book.status} name="status">
                        <option value="to_read">To read</option>
                        <option value="reading">Reading</option>
                        <option value="finished">Finished</option>
                      </select>
                      <button className="ml-2 text-xs font-medium text-[#05386B]" type="submit">Save</button>
                    </form>
